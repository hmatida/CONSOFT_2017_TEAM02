	...
	<div class="col-md-12">
		<div class="container">
			...
			<form method="post" action="/evento/confList">
				 <label for="conferencia" name="conferencia">Selecione a conferencia</label>
					 <select name="conferencia">
					 <c:forEach items="${conferencia}" var="conf">
					 	<option value=${conf.id}>${conf.nome_abre}</option>
					 </c:forEach>
				 </select>
				 <button type="submit" class="btn btn-default" action="/evento/confList" href=/evento/confList/${conf.id}>
					Buscar
				</button>
			 </form>
